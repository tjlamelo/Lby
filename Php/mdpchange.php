<?php   
    ...
    if(!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['new_password_retype'])){
        ...
        $check_password  = $bdd->prepare('SELECT password FROM insccription WHERE token = :token');
        ...
        if(password_verify($current_password, $data_password['password']))
        {
            ...
            if($new_password === $new_password_retype){
                ...
                $update = $bdd->prepare('UPDATE inscription SET password = :password WHERE token = :token');
                $update->execute(array(
                    "password" => $new_password,
                    "token" => $_SESSION['user'];
                ));
                // On redirige
                header('Location: ../Pages/profil.php?err=new_password_retype');
                die();
            }
            else{
                header('Location: ../Pages/profil.php?err=current_password');
            die();
            }
        }
        else{
